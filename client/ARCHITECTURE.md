...
└── data/                ← 硬编码常量（suggestions / DEFAULT_MODELS）
```
...
```
┌───────────────────────────────────────────────────────────────┐
│ 1-45    import：组件、hook、工具函数                            │
├───────────────────────────────────────────────────────────────┤
│ 47      function App()  ← 应用根组件                           │
│                                                               │
│ 48-140  ① 状态声明区                                           │
│ 144-168 ② 启动初始化：拉后端模型列表                            │
│ 175     ③ 同步浏览器地址栏颜色（useThemeColorSync）             │
│ 181-335 ④ Agent SSE 重连 effect（最复杂的一块）                 │
│ 341-407 ⑤ 可见性兜底（手机后台漏事件的补救）                    │
│ 409-467 ⑥ 零碎 effects：聚焦/滚动/textarea 自适应高度           │
│ 469-477 ⑦ 卸载清理                                            │
│                                                               │
│ 479-538 ⑧ 三个业务 hook：chat / session / agent 传输           │
│ 540-561 ⑨ handleSubmit / handleKeyDown                        │
│ 563-614 ⑩ 派生数据 + 工具栏 slot 变量                          │
│                                                               │
│ 616-758 ⑪ return (...)：实际渲染的 JSX                        │
└───────────────────────────────────────────────────────────────┘
```
...
挂载时 `fetch('/api/models')`：
1. 更新 `availableModels`；
2. 顺手扫一遍历史会话，把引用了已下线模型的 session 改成首个可用模型；
3. 设 `modelsLoaded = true`，让"清理用户多选模型"effect 才敢动手——否则启动一瞬间会用占位列表过滤，把用户已选的多模型裁掉。

### ③ `useThemeColorSync`（175）

把 `<meta name="theme-color">` 改成当前主背景色，让手机浏览器顶部地址栏跟着变色。逻辑全在 hook 里。

### ④ Agent SSE 重连 effect（181-335）⭐
...
| `hooks/usePersistentState.js` | `useState` 的 localStorage 版（含 boolean / json 序列化策略） |
...
### 加一个新的后端接口
1. 在 `api/streams.js` 加封装函数（或新建 `api/foo.js`）。
