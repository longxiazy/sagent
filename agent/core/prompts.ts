...
export function buildNvidiaTaskMessages({
    ...
    return [
      {
        ...
        content: [
          ...
          `字符串值内部的双引号必须转义为 \\"；shell 命令用单引号包裹参数，如 "command":"grep -n 'foo bar' a.txt"。`,
  ...
  const outputProtocolLines = nativeTools
    ...
    : [
        ...
        `3. 字符串值内部的双引号必须转义为 \\"，换行必须转义为 \\n，否则整个 JSON 无法解析；shell 命令一律用单引号包裹参数，例如 {"tool":"terminal","type":"run_safe","command":"echo 'done' && grep -n 'foo bar' a.txt"}。`,
        '4. rationale 只说明当前动作的直接目的，必须简短，不要输出详细推理过程。',
        '5. 每次只能选择一个已启用动作；action 必须严格遵循下方示例及签名，不得添加未定义字段。',
        '6. 每个 action 都必须包含 tool 和 type；finish 固定使用 {"tool":"core","type":"finish","answer":"最终结果"}。',
        '代表性动作示例：',
