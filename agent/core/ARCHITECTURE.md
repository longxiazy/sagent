...
`agent/core/` 是 Agent 运行时的心脏：**不依赖任何具体工具实现与网络细节**，通过注入函数（initialize/observe/decide/authorize/execute/cleanup）与外部组合，同时提供全部共享的状态仓库、配置边界与纯函数工具。
...
│  ai-client.ts · action-types.ts · schemas.ts                  │
├─────────────────────────────────────────────────────────────┤
│  供应商目录（不在此列）                                       │
...
- **contracts.ts** — 全仓库共享的核心类型：`AgentAction`（各工具动作的判别联合）、`AgentEvent`（SSE 事件）、`AgentStep`、`RunStatus` 状态机（`RUN_STATUS_TRANSITIONS`）、`RunRecord`、`RuntimeContext` 系列、`ApproveStore`/`AgentRunStore` 接口。外部模型/SDK 的未知数据必须在 adapter/schema 边界被校验，进入 runtime 后只使用这里的归一化类型。
...
- **router.ts** — `createActionRouter()`：把归一化的 action 按 `tool` 字段分发到对应 handler，缺失时抛错。

### 4. 决策层（提示词 → 模型 → 归一化）

- **prompts.ts** — 提示词构造中枢：按任务/历史关键词推断能力并选择工具（`selectGeminiToolNames`），构造 NVIDIA 与 Gemini 两套消息（全量/compact/finalOnly/原生 tool-call 四变式），内置规则与防原地踏步提示（最近 URL/搜索提示）。
- **tool-definitions.ts** — 全部工具的 schema 定义（含示例与规则说明），运行时按 Chrome/MCP 可用性动态注入，`toolToGeminiTool`/`toolToOpenAiTool` 转两种供应商格式。
...
- **summarizer.ts** — provider `summarize()` 的封装（框式日志 + 异常重抛）。
...
- **result-quality.ts** — 运行质量评估：官方来源缺失 → unverified，浏览意图无有效观测/失败步 → degraded。
- **trace-replay.ts** — trace 事件重建 `ReplayRun`（与 runtime 组装 history 口径一致），供离线评测。
...
runtime ──→ router / checkpoint / result-quality / result-extraction / config-store
session-store ──→ project-store / trace-replay / persistence-queue
...
- 新供应商：`providers/` 新增并按 registry 要求注册，缩写为 `ai-client.deriveProviderName`。
- 新工具：`agent/action-types.ts` 加映射 → `contracts.ts` 加 `AgentTool`/`AgentAction` → `tool-definitions.ts` 加 schema → `schemas.ts` 加归一化。
